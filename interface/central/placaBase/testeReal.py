...
PlacaBase.iniciar(porta='/dev/ttyACM0', taxa=115200, callback=c)
tempo = 1

# PlacaBase.enviaComando('3', 'CHANGE_OUTPUT_STATE', (256,))
# PlacaBase.fechar()
# exit()

while(False):
    try:
        PlacaBase.enviaComando('3', 'CHANGE_OUTPUT_STATE', (randint(0,255),))
        # for x in range(8):
...
while(True):
    try:
        ...
        if(entrada == '1'):
            PlacaBase.enviaComando('3', 'CHANGE_ID', '50')
        if(entrada == '2'):
            PlacaBase.enviaComando('3', 'CHANGE_OUTPUT_STATE', (0,))
        if(entrada == '3'):
            PlacaBase.enviaComando('3', 'CHANGE_SEND_TIME', "10")
        if(entrada == '4'):
            PlacaBase.enviaComando('50', 'CHANGE_SEND_TIME', "255")
            PlacaBase.enviaComando('50', 'CHANGE_SEND_TIME', "255")
        if(entrada == '5'):
            PlacaBase.enviaComando('50', 'IS_ONLINE')
        if(entrada == '-'):
