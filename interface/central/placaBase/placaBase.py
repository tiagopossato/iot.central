...
class PlacaBase():
    ...
    def fechar():
        try:
            ...
            try:
                signal.pthread_kill(PlacaBase._thMonitora.ident, signal.SIGKILL)
            ...
            try:
                signal.pthread_kill(PlacaBase._thCallback.ident, signal.SIGKILL)
    ...
    def run(self):
        try:
            ...
            while(PlacaBase._bufferEnvio.empty() == False):
                ...
                # print("PlacaBase._bufferEnvio: " + str(PlacaBase._bufferEnvio))
                # print("--------------------------------------")
                while(PlacaBase._portaSerial.isOpen() == False): pass
...
class _MonitoraPlacaBase(Thread):
    ...
    def run(self):
        ...
        while(True):
            try:
                PlacaBase._isOnline = False
                #PlacaBase.enviaComando(CENTRAL_ID, 'IS_ONLINE')
